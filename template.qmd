...
1. There are -999 in some spots where it should be a null value 
2. There are 1,2, and 3 in the variety area. 
...
```{r}
LongKern <- seeds_df|> 
  pivot_longer(cols="a":"l_g",
               names_to = "name",
               values_to = "value")

Hist1<- ggplot(LongKern, aes(x=value))+ 
...
Scatter 1
```{r}
Scat1 <- ggplot(seeds_df,aes(x=a, y = asym, color = variety))+ 
  geom_point()+ 
  labs(x="Area", y="Assymetry")+
  theme_bw()
Scat1
```

```{r}
Scat2 <- ggplot(seeds_df,aes(x=l_k, y = w_k, color = variety))+ 
  geom_point()+ 
  labs(x="Length of Kernel", y="Width of Kernel")+
  theme_bw()
Scat2
```
...
seeds_scale <- seeds_complete |>
  select(-variety)|> #drop variety column 
  scale()
```
...
```{r}
#| fig-cap: Optional Number of Clusters for Kernels. After adding the 3rd cluster there is less of a drop off so we will use 3 groups for our kmeans cluster. 
...
set.seed(10101)
seeds_km <- kmeans(seeds_scale,3,iter.max = 10)# kmeans specifying 3 groups to start
summary(seeds_km)
```
...
SeedsScat <- ggplot(seeds_cl,aes(x= a, y = asym, color = cluster_no, shape = variety))+ 
  geom_point()+ 
  labs(x="Kernel Area", y="Assymetric Coefficient")+
  theme_bw()
SeedsScat
```
...
• Most of the observations can be defined by their variety but there is some overlap between Kama and the others. 


Can you make a table to show the comparison between the variety and defined clusters?

```{r}
#| tbl-cap: Contiency table for the groupings of kmeans and the varietals of kernels 
...
2. drop na's (don't have any in this df)
...
rownames(wb_scaled) <- wb_ghg_20$name #allows us to put the names into where the observation # is, not new column
```
